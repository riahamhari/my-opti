...
interface Course {
	...
	'Instructor Image': string;
	'Content Level': string;
	'Modules Inside': number;
	'Time Commitment': string;
	'Time to Finish': string;
	'Perfect For': string;
}
...
(() => {
	...
	const courseInfo: CourseInfo = {
		...
		'Wiring Fundamentals': {
			...
			'Instructor Image':
				'https://www.hpacademy.com/assets/Taz-Articles-and-Blogs/PDM/f0943aa0e3/zac-hpa2__ResizedImageWzc4Miw0OTJd.jpg',
			'Content Level': 'Essential Knowledge',
			'Modules Inside': 66,
			'Time Commitment': '10-30 Minutes a Day',
			'Time to Finish': '3 weeks',
		...
		'Practical Diesel Tuning': {
			...
			'Instructor Image':
				'https://i.ytimg.com/vi/gpkdhRQDmg8/hq720.jpg?sqp=-oaymwEhCK4FEIIDSFryq4qpAxMIARUAAAAAGAElAADIQj0AgKJD&rs=AOn4CLDrUDmDA06f3tG9IZHWAgQK2pCwZA',
			'Content Level': 'Experienced',
			'Modules Inside': 52,
			'Time Commitment': '10-30 Minutes a Day',
			'Time to Finish': '2–3 weeks',
		...
		'Suspension Tuning & Optimization': {
			...
			'Instructor Image':
				'https://media.licdn.com/dms/image/v2/C5603AQFeYbeT7qJ2aA/profile-displayphoto-shrink_200_200/profile-displayphoto-shrink_200_200/0/1633393000742?e=2147483647&v=beta&t=sTHuaekGxm-thVk4s4YhlXbB78vWrRklpCTqXXqgjUw',
			'Content Level': 'Essential Knowledge',
			'Modules Inside': 38,
			'Time Commitment': '10-30 Minutes a Day',
			'Time to Finish': '2 weeks',
		...
		'Wheel & Tyre Fitment': {
			...
			'Instructor Image':
				'https://www.hpacademy.com/assets/Uploads/team-members/b52e5af09e/Staff-Photos-About_Connor__FillWzMyNywyOTld.png',
			'Content Level': 'Essential Knowledge',
			'Modules Inside': 35,
			'Time Commitment': '10-30 Minutes a Day',
			'Time to Finish': '1–2 weeks',
		...
		'Race Driving Fundamentals': {
			...
			'Instructor Image':
				'https://i.vimeocdn.com/video/1170200604-f649e107a8fa1ecece3b26bd8cecade6974bf7e18670a526b014ff66f3ef5fb0-d_640x360?r=pad',
			'Content Level': 'Essential Knowledge',
			'Modules Inside': 54,
			'Time Commitment': '10-30 Minutes a Day',
			'Time to Finish': '2–3 weeks',
		...
		'EV Fundamentals': {
			...
			'Instructor Image':
				'https://www.hpacademy.com/assets/Uploads/blog-posts/1878342509/sasha-anis__FillWzYwMCw0MDBd.jpg',
			'Content Level': 'Essential Knowledge',
			'Modules Inside': 40,
			'Time Commitment': '10-30 Minutes a Day',
			'Time to Finish': '2 weeks',
	...
	const myInterval = setInterval(() => {
		const titleAnPriceBlock: HTMLElement = document.querySelector('.tittleAndPriceBlock');
	...
	const optiInit = (titleAnPriceBlock: HTMLElement) => {
	...
	const createInfoBar = ({
		...
		const createInfoItem = (icon: string, label: string, value: string, additionalText?: string) => {
			return `
				<div class="opti_course_info_item">
					<span class="opti_course_info_item__icon"><img src="${icon}" /></span>
					<div class="opti_course_info_item_details">
						<p class="opti_course_info_item__label">${label}</p>
						<p class="opti_course_info_item__value">${value}</p>
						${additionalText ? `<p class="opti_course_info_item__demand">${additionalText}</p>` : ''}
					</div>
				</div>
			`;
		};
		...
		infoBar.innerHTML = `
			<div class="opti_course_info_instructor">
				<img src="${instructorImage}" alt="Instructor ${instructorName} image" />
				<div class="opti_course_info_instructor__details">
					<p class="opti_course_info_instructor__label">Instructor</p>
					<p class="opti_course_info_instructor__name">${instructorName}</p>
				</div>
			</div>
			<div class="opti_course_info">
				${createInfoItem(icons.level, 'Content Level', contentLevel)}
				${createInfoItem(icons.video, 'Modules Inside', `${modulesInside} Video Lessons`)}
				${createInfoItem(icons.time, 'Time Commitment', timeCommitment, '(on-demand)')}
				${createInfoItem(icons.calendar, 'Time to Finish', timeToFinish)}
				${createInfoItem(icons.person, 'Perfect for', perfectFor)}
