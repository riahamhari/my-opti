...
(() => {
	...
	const myInterval = setInterval(() => {
		const positionEl =
			document.querySelector('.CourseEntry .courses-info-box-wrapper') ||
			document.getElementById('vip-courses-future') ||
			document.querySelector('.main-content:has(.video-mask) h3');
		const coursePrice =
			document.querySelector<HTMLElement>('.new-price') ||
			document.querySelector<HTMLElement>('.total span') ||
	...
	const optiInit = () => {
		...
		const coursePrice = coursePriceEL?.innerText.split(' ')[0];
	...
	const createTable = (isCoursePage: boolean, courseName: string, key = undefined, price) => {
	...
	const createPriceBtn = (enrollBtn) => {
		const eightPaymentsTxt = enrollBtn.querySelector('span')?.innerText;
		...
		const ctaBtn: HTMLElement = ctaWrapper.querySelector('.btn');
