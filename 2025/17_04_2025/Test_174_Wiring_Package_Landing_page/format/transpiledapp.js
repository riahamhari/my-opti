...
(() => {
    ...
    const discountLabel = `<div class="opti_discount_label">$${!1}{icons.outlineDollar} <span>Over 60% Off</span></div>`;
    const copyObj = {
    ...
    const wiringBenefits = [
        ...
        `24 Months Gold Membership <div class="popover-trigger">$${!1}{icons.infoTt} $${!1}{lifetimeToolTipHtml}</div>`,
...
`;
    ...
    const optiInit = () => {
        ...
        ctaContainer.insertAdjacentHTML('beforebegin', `<div>$${!1}{copyObj.aboveHeroCta}</div>`);
    ...
    const insertHeroTicks = (container) => {
        const heroTicks = `<div>$${!1}{icons.heroTick}
                               <p>Lifetime access</p>
        </div>
		<div>$${!1}{icons.dollarSign}
    ...
    const insertYoullLearnUpdate = (wrapper) => {
        ...
        container.innerHTML = `<div>
									$${!1}{discountLabel}
									<div>$${!1}{copyObj.aboveHeroCta}</div>
            ...
            .join('')}
									...
									<ul class="opti_ultimate_wiring__incl">
										$${!1}{wiringIncludes.map((include) => `<li>$${!1}{include}</li>`).join('')}
									</ul>
    ...
    const createTable = () => {
        ...
        const tableHtml = `<table id="opti_price_comparison_table">
            ...
            .map(({ feature, included }) => `<tr>
											<td>$${!1}{feature}</td>
											<td>$${!1}{icons.tick}</td>
											<td>$${!1}{included ? icons.tick : icons.cross}</td>										
										</tr>`)
            .join('')}
