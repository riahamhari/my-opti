...
import { Course, CoursesData, VideoContent } from './types';
...
(() => {
	...
	const myInterval = setInterval(() => {
		const enrollBtn: HTMLAnchorElement = document.querySelector('.btn-wrapper .btn');
		if (enrollBtn) {
	...
	const copyForCoursePages: CoursesData = {
		'tuning starter package': {
			...
			videoInfo: {
				...
				vimeoId: '557409982',
				imageSrc:
					'https://i.vimeocdn.com/video/1152957103-53cebbcc800d670d4804c878b7fd293e43174b8d012dd92971f4774e183ad168-d_960x540?r=pad',
			},
		},
		'wiring starter package': {
			...
			videoInfo: {
				...
				vimeoId: '294276966',
				imageSrc:
					'https://i.vimeocdn.com/video/1084542389-cf58b324c9c4677a6931c41c7c37e111be42623fcf7a8269d4c441e9811cbb68-d_960x540?r=pad',
			},
		},
		'engine building starter package': {
			...
			videoInfo: {
				...
				vimeoId: '229938870',
				imageSrc:
					'https://i.vimeocdn.com/video/1349796295-36871d6dfbf75a0d3688641ec4dfa06adf97e0e0fda0a4708018d25977ccc275-d_960x540?r=pad',
			},
		},
		'diesel tuning starter package': {
			...
			videoInfo: {
				...
				vimeoId: '394300677',
				imageSrc:
					'https://i.vimeocdn.com/video/860118533-cd211c7521efa23b741b48812123c452cdf5e977d800b043d78df3395e9c2497-d_960x540?r=pad',
			},
		},
		'track day package': {
			...
			videoInfo: {
				...
				vimeoId: '579226829',
				imageSrc:
					'https://i.vimeocdn.com/video/1198489022-8e6106394944e17fbb28690e7ebd7b60cfe3fa0ca64cf55c1cb738e0bd169217-d_960x540?r=pad',
			},
		},
		'fabrication package': {
			...
			videoInfo: {
				...
				vimeoId: '673340115',
				imageSrc:
					'https://i.vimeocdn.com/video/1398466820-4939383c651822380a2aa46e6798fa0f2c6391c6ca71b51aa6fe3a4a2924bce7-d_960x540?r=pad',
			},
		},
	};

	const optiInit = (enrollBtn: HTMLAnchorElement) => {
		const currentUrl = window.location.href;
		const enrollBtnUrl = enrollBtn.href;

		const baseUrl = new URL(currentUrl).origin + new URL(currentUrl).pathname;

		const key = Object.keys(copyForCoursePages).find((course) =>
			copyForCoursePages[course].courseLandingPages.some(
		...
		const matchingCourse: Course = copyForCoursePages[key];
	...
	const createNewBtn = (enrollBtn, modal: HTMLElement) => {
		...
		const modalMain = modal.querySelector<HTMLElement>('.info-modal');
	...
	const createModal = (
		{ packageToLinkTo, saving, packageIncludes, price, totalValue, videoInfo }: Course,
		courseName: string,
		enrollBtnUrl: string
	): HTMLElement => {
		...
		const overlay = modal.querySelector<HTMLElement>('.modal');
	...
	function generateVideoHtml({ href, vimeoTitle, vimeoId, imageSrc }: VideoContent): string {
		return `
	...
	const generatePriceBlockHtml = (price: string, totalValue: string) => {
