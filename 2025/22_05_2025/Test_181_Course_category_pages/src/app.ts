...
(() => {
	...
	const optiInit = () => {
		...
		courseCards.forEach((card) => {
			...
			const viewCourseBtn: HTMLAnchorElement = card.querySelector('.btn-primary');
			const defaultBtn: HTMLAnchorElement = card.querySelector('.btn-default');

			const bannerTitle: HTMLElement = card.querySelector('.courseSeries__preview__banner h4');
			if (bannerTitle) {
