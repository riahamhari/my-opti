...
(() => {
	...
	const myInterval = setInterval(() => {
		const packageTitle: HTMLElement = document.querySelector('.tittleAndPriceBlock__title');
		const packageEl = document.querySelector('.package');
		const buttonWrapper: HTMLElement = document.querySelector('.btn-wrapper');
		const desktopPackageTotal = document.querySelector('.sidebar.col-sm-4  .package .package-total');
		const paymentPlanHtml = document.querySelector<HTMLElement>('.titleAndPriceBlock__paymentPlanLink');
		if (packageTitle && packageEl && buttonWrapper && desktopPackageTotal && paymentPlanHtml) {
			clearInterval(myInterval);
			optiInit(packageTitle, packageEl);
		}
	}, 300);

	const optiInit = (packageTitle: HTMLElement, packageEl) => {
		const packageInclusion = packageEl.cloneNode(true) as HTMLElement;
		...
		const desktopPackageCourses = [...packageEl.querySelectorAll('.package-course')];

		const lastPackage = packageCourses[packageCourses.length - 1];
		const desktopLastPackage = desktopPackageCourses[desktopPackageCourses.length - 1];

		const lastPackageTitle = lastPackage.querySelector('h4') as HTMLElement;
		const desktopLastPackageTitle = desktopLastPackage.querySelector('h4') as HTMLElement;
		const lastPackagePriceEl = lastPackage.querySelector('.course-price') as HTMLElement;
		const lastPackagePrice = lastPackagePriceEl?.innerText;
		if (lastPackageTitle.innerText === '24 Months of Gold Membership' && lastPackagePrice) {
			...
			const lastPackageCopy = `PLUS: bonus 2 years of dedicated support and webinar access <span>worth ${lastPackagePrice}</span>`;
			lastPackageTitle.innerHTML = lastPackageCopy;

			desktopLastPackage.classList.add('opti_2_years_support');
			desktopLastPackageTitle.innerText = `PLUS: bonus 2 years of dedicated support and webinar access`;
		}
		packageTitle.insertAdjacentElement('afterend', packageInclusion);
		// add new cta
		const desktopPackageTotal = document.querySelector('.sidebar.col-sm-4 .package .package-total');
		const newCtaHtml = document.querySelector('.sidebar.tittleAndPriceBlock').innerHTML;
		const newCta = document.createElement('div');
		newCta.classList.add('opti_new_cta');
		newCta.innerHTML = newCtaHtml;

		desktopPackageTotal.insertAdjacentElement('afterend', newCta);

		// add 8 payments html to desktop

		const paymentPlanHtml = document.querySelector<HTMLElement>('.titleAndPriceBlock__paymentPlanLink').innerHTML;
		const eightPaymentsHtml = `<div class="opti_desktop_or">-OR-</div>
									<div class="opti_desktop_payment_plan">${paymentPlanHtml}
									<div class="popover fade bottom in" role="tooltip" id="opti_popover_vip">
		<div class="arrow"></div>
		...
		desktopPackageTotal.insertAdjacentHTML('afterend', eightPaymentsHtml);

		// popover fucntionality
		const waitForTrigger = setInterval(() => {
			const popoverTrigger = document.querySelector('.opti_desktop_payment_plan .popover-trigger');
			const popover = document.querySelector('.opti_desktop_payment_plan .popover');
...
(() => {
	...
	const myInterval = setInterval(() => {
		...
		const mainCta: HTMLElement = document.querySelector('.btn-wrapper');
		const priceEl: HTMLElement = document.querySelector('.main-content .price');

		if (vipCoursesCurrent && mainCta) {
			clearInterval(myInterval);
			optiInit(mainCta, priceEl);
		}
	}, 300);

	const optiInit = (mainCta: HTMLElement, priceEl: HTMLElement) => {
		const price = priceEl.innerText;
		const packageEl = document.querySelector('.package');
		const defaultBtn: HTMLElement = document.querySelector('.btn-wrapper');
		defaultBtn.classList.remove('btn-sticky');

		const packageTitle = document.querySelector('.starter-title');
		packageTitle.id = 'opti_starter_title';
		const eightPaymentsText = mainCta.querySelector('span').innerText.toLowerCase();
		...
		const priceHtml = `<div class="col-sm-12 col-md-6 tittleAndPriceBlock__price opti_title_price">
			...
			<span class="titleAndPriceBlock__paymentPlanLink">
				...
				<a class="line1 js_paymentPlanPurchase" href="/vip-package/#">${eightPaymentsText}</a>
				<a class="line2 js_paymentPlanPurchase" href="/vip-package/#"
					>Instant access. Easy checkout. No fees.</a
				>
				<span
					class="popover-trigger opti_popover_trigger show-button"
					>Learn more</span
				>
				${popoverHtml}
			</span>
		...
		const newCta = `<div class="sidebar col-sm-4 tittleAndPriceBlock">
			<form
				class="js_gaPurchaseForm"
				data-parsley-validate=""
				role="form"
				id="ComboPurchaseForm_ComboPurchaseForm"
				action="/vip-package/ComboPurchaseForm/"
				method="post"
				...
				<fieldset>
					<input
						type="hidden"
						name="OpenPaymentPlanOptions"
						value="0"
						class="combohidden"
						id="ComboPurchaseForm_ComboPurchaseForm_OpenPaymentPlanOptions_0" />
					<div class="row">
						<div class="col-sm-12">
							<div class="btn-wrapper opti-btn-wrapper">
								<button
									type="submit"
									form="ComboPurchaseFormWithVariationID_0"
									class="btn btn-submit btn-primary btn-large js-addProductDetail js-ga_addProductToCart loggedIn-starterPackage"
									data-ga_productinfo='{"id":"7","name":"HPA Starter Package","price":"417.00","quantity":1,"category":"StarterPage"}'
									data-gtm-products='[{"id":"7","name":"HPA Starter Package","price":"417.00","category":"StarterPage","brand":"","variant":""}]'
									data-backurl="/vip-package/">
									Enroll Now

									<br />
									<span>${eightPaymentsText}</span>
								</button>
			...
			<div class="opti_tittleAndPriceBlock__points">
				<h5 class="guarantee">
				...
				<h5 class="paymentPlan">
					<img
						width="22"
						height="22"
						src="https://www.hpacademy.com/assets/Icons/29e09dc390/noun-key-7324260-808184.png" />
					...
					<div class="opti_relative">
						...
						<div class="popover fade bottom in" role="tooltip" id="opti_lifetime_popover">
							<div class="arrow"></div>
							<h3 class="popover-title" style="display: none;"></h3>
							<div class="popover-content">
								<p>
									High Performance Academy has been in business for 12 years, we have grown to 14 staff, two workshops,
									two studios and from a small startup to a professional organisation.
								</p>
								<p>Suffice to say, we aren’t going out of business any time soon.</p>
								<p>
									However, in the unlikely scenario we close, we will continue to guarantee you lifetime access with a
									download link to all of our courses.
								</p>
							</div>
						</div>
					</div>
		...
		packageTitle.insertAdjacentElement('beforeend', packageInclusion);

		packageTitle.insertAdjacentHTML('beforeend', priceHtml);
		packageTitle.insertAdjacentHTML('beforeend', newCta);

		// add new cta
		const desktopPackageTotal = document.querySelector('.sidebar.col-sm-4 .package .package-total');
		const newDesktopCtaHtml = document.querySelector('.sidebar .btn-wrapper').innerHTML;
		const newDesktopCtaVip = document.createElement('div');
		newDesktopCtaVip.classList.add('opti_new_cta');
		newDesktopCtaVip.innerHTML = newDesktopCtaHtml;

		desktopPackageTotal.insertAdjacentElement('afterend', newDesktopCtaVip);
		...
		// popover fucntionality
		const waitForTrigger = setInterval(() => {
			const popoverTrigger = document.querySelector('.opti_title_price .opti_popover_trigger');
			const desktopTrigger = document.querySelector('.opti_desktop_payment_plan .opti_popover_trigger');
			const desktopPopover = document.querySelector('.opti_desktop_payment_plan .popover');
			const popover = document.getElementById('opti_popover_vip');
			if (popoverTrigger && popover) {
				clearInterval(waitForTrigger);
				popoverTrigger.addEventListener('click', function (e) {
					e.preventDefault();
				...
				desktopTrigger.addEventListener('click', function (e) {
					e.preventDefault();
					desktopPopover.classList.toggle('opti_show_popover');
				});
				...
				document.addEventListener('click', function (e: any) {
					...
					if (!desktopPopover.contains(e.target) && !desktopTrigger.contains(e.target)) {
						desktopPopover.classList.remove('opti_show_popover');
					}
				});
			}
		}, 300);

		// popoverIcons triggers

		const waitForIconTrigger = setInterval(() => {
			const popoverIconTriggers = [
				...document.querySelectorAll<HTMLElement>('.opti_tittleAndPriceBlock__points .opti_info_icon'),
			];

			if (popoverIconTriggers && popoverIconTriggers.length) {
				clearInterval(waitForIconTrigger);

				popoverIconTriggers.forEach((popoverTrigger) => {
		...
		const optiButtonWrapper: HTMLElement = document.querySelector('.opti-btn-wrapper');
		const buttonBottomOffset = optiButtonWrapper.offsetTop + optiButtonWrapper.offsetHeight;
		...
		window.addEventListener('scroll', () => {
			...
			// Check if the button is completely out of view
			if (window.scrollY >= buttonBottomOffset + 1280) {
				defaultBtn.classList.add('btn-sticky');
			} else {
				defaultBtn.classList.remove('btn-sticky');
			}
		});
