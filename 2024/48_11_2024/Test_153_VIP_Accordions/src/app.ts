...
<g id="SVGRepo_iconCarrier"> <path d="M20 4L12 12M20 4V8.5M20 4H15.5M19 12.5V16.8C19 17.9201 19 18.4802 18.782 18.908C18.5903 19.2843 18.2843 19.5903 17.908 19.782C17.4802 20 16.9201 20 15.8 20H7.2C6.0799 20 5.51984 20 5.09202 19.782C4.71569 19.5903 4.40973 19.2843 4.21799 18.908C4 18.4802 4 17.9201 4 16.8V8.2C4 7.0799 4 6.51984 4.21799 6.09202C4.40973 5.71569 4.71569 5.40973 5.09202 5.21799C5.51984 5 6.07989 5 7.2 5H11.5" stroke="#808184" stroke-width="1.8719999999999999" stroke-linecap="round" stroke-linejoin="round"/> </g>
	...
	const coursesData: CoursesData = {
		tuning: {
			...
			courseImgSrc:
				'https://www.hpacademy.com/assets/f5b480d6cd/Screen-Shot-2020-08-19-at-11__ResizedImageWzE1NiwxNDBd.02.58-AM.png',
			courses: [
				{
    ...
    `;
			...
			courses.forEach((course) => {
				const studentsEnrolled =
					course.title === 'Understanding AFR' ? '8,564 Students enrolled' : '11,675 Students enrolled';
            ...
            <li class="opti_course_list_item">
                <div class="opti_course_details">
                    ...
                    <p><span class="opti_course_modules">${course.lessons.replace(
											'Lessons',
											'Modules'
										)}</span> • <span class="opti_course_students">${studentsEnrolled}</span></p>
                </div>
	...
	const accordionFunction = () => {
		const accordions = document.querySelectorAll<HTMLElement>('#opti_accordion_list .accordion');
		...
		accordions.forEach((accordion: HTMLElement) => {
			...
			const accordionContentWrap = accordion.querySelector<HTMLElement>('.accordion__content-wrap');

			// Ensure content wrap exists before proceeding
			if (!heading || !accordionContentWrap) return;
			...
			heading.addEventListener('click', function () {
				const parent = this.parentNode as Element; // Assert that parentNode is an Element
		...
		if (expandAllBtn) {
			expandAllBtn.addEventListener('click', function () {
				...
				accordions.forEach((accordion: HTMLElement) => {
					const accordionContentWrap = accordion.querySelector<HTMLElement>('.accordion__content-wrap');
					const parent = accordion as Element; // Treat accordion as the parent
	...
	function handleClass(node: Element, className: string, action: 'add' | 'remove' = 'add') {
