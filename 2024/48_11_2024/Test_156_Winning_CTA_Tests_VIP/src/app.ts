...
(() => {
	...
	const myInterval = setInterval(() => {
		const mainCtaCol: HTMLElement = document.querySelector('.sidebar:has(.btn-wrapper)');
		const mainBtnWrapper: HTMLElement = document.querySelector('.sidebar .btn-wrapper');
		...
		const courseTitle: HTMLElement = document.querySelector('.starter-title h2');
	...
	const optiInit = (priceWrapper: HTMLElement, mainCtaCol: HTMLElement) => {
		const btnWrapper: HTMLElement = mainCtaCol.querySelector('.btn-wrapper');
		...
		const newBtn = btnWrapper.cloneNode(true) as HTMLElement;
		...
		const eightPaymentsCont = `
								...
								"
									...
									>
										${popoverHtml}
								`;
		const eightPaymentsEl = createElement(
			'span',
			'titleAndPriceBlock__paymentPlanLink',
			eightPaymentsCont + popoverHtml
		);
		...
		titleAndPriceBlock.insertAdjacentHTML(
			'afterbegin',
	...
	const addPopoverHandlers = (eightPaymentsEl: HTMLElement) => {
	...
	const updateToolTipPosition = () => {
		const relativeEl: HTMLElement = document.querySelector('#opti_price_wrapper .titleAndPriceBlock__paymentPlanLink');
		const relativeElHeight = relativeEl.offsetHeight;
		const popover: HTMLElement = document.querySelector('#opti_price_wrapper #opti_popover');

		popover.style.top = `${relativeElHeight + 4}px`;
	};

	const createElement = (element: string, classes, htmlContent: string) => {
		if (!element) return;
