...
(() => {
	...
	const myInterval = setInterval(() => {
		const priceBlock = document.querySelector<HTMLElement>('.tittleAndPriceBlock');
		const priceBlockPrice = document.querySelector<HTMLElement>('.tittleAndPriceBlock .tittleAndPriceBlock__price');
	...
	const optiInit = (priceBlock: HTMLElement, priceBlockPrice: HTMLElement) => {
		const currentUrl = window.location.href;

		const baseUrl = new URL(currentUrl).origin + new URL(currentUrl).pathname;

		const matchingCourse = Object.keys(copyForCoursePages).find((course) =>
		...
		const upsellHtml = `<p id="opti_starter_package_upsell_desktop">${copy} <a href=${packageLink} class="opti_package_link">Learn More.</a></p>`;

		const mobileUpsellHtml = `<p id="opti_starter_package_upsell_mobile">${copy} <a href=${packageLink} class="opti_package_link">Learn More.</a></p>`;
