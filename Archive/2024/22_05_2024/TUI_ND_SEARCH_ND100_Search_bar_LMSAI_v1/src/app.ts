...
(function optiIfee() {
	...
	const myInterval = setInterval(function () {
		const tabsContainer: HTMLUListElement = document.querySelector('.UI__choiceSearchTabsContainer ul');
		if (tabsContainer) {
			...
			const replaceText = (): void => {
				tabs.forEach((el) => {
					const tabLabel: HTMLSpanElement = el.querySelector('div.UI__choiceSearchTab > span.UI__label');
			...
			function getThreshold(language): number {
				switch (language) {
					case 'se':
						return 476;
					case 'dk':
