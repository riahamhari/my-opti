...
<g stroke="#1B115C" stroke-width="2" fill="none" fill-rule="evenodd" stroke-linecap="round" stroke-linejoin="round">
    ...
    const optiInit = () => {
        ...
        const totalPrice = extractPrice(store.getState().boards[`$${!1}{selectedId}`].price.total);
        const waitForCabins = setInterval(() => {
            ...
            const addMoreCabinsBtn = document.querySelector('#roomAllocation__component .RoomAllocation__buttonContainer button');
            const perPersonPrice = extractPrice(store.getState().boards[`$${!1}{selectedId}`].price.perPerson);
            const boardOptionsCont = document.querySelector('#boardBasisBreakdown__component .WCMS_component');
            const sectionHeading = document.querySelector('#boardBasisBreakdown__component .sectionHeading');
            if (roomListEl && numOfCabins && addMoreCabinsBtn && perPersonPrice && boardOptionsCont && sectionHeading) {
                clearInterval(waitForCabins);
                const premiumAllInc = `<div class="opti_premiumAI_container $${!1}{isPremiumSelected ? 'opti_premium_selected' : ''}" aria-label='premium all inclusive list'>
										...
										<div class="opti_premiumAI_included">
											<ul>
												$${!1}{whatsIncludedArr
                    .map((item) => {
                    return `<li class="opti_included_list_item">$${!1}{checkMarkIcon} <span class="opti_included_list_item_details">$${!1}{item}</span></li>`;
                })
                    .join('')}
											</ul>
										</div>
										<div class="opti_premiumAI_selection">
											<p class="opti_premiumAI_price"><span class="opti_pp_price">$${!1}{perPersonPrice}pp</span> per night <span class="opti_tt_price">($${!1}{totalPrice})</span></p>
											$${!1}{removeLink} $${!1}{selectBtn}
                ...
                if (!roomListEl.querySelector('.opti_num_of_cabin_listItem')) {
                    roomListEl.insertAdjacentHTML('afterbegin', `<li class="opti_num_of_cabin_listItem">$${!1}{cabinIcon}<p class="opti_number_of_cabins">Number of cabins: <span>$${!1}{numOfCabins}</span></p></li>`);
                    addMoreCabinsBtn.innerText = 'Add more cabins';
                }
                else {
                    const numOfCabinEl = document.querySelector('.opti_number_of_cabins span');
                    numOfCabinEl.innerText = `$${!1}{numOfCabins}`;
                }
