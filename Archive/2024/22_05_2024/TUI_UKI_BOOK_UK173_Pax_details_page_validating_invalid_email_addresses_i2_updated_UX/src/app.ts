...
(function optiIfee() {
	...
	const myInterval = setInterval(function () {
		const leadPaxSection: HTMLElement = document.querySelector('[aria-label="ADULT 1"]');
		const importantInfoEl = document.querySelector('#importantInformation__component');
		const emailInputField: HTMLInputElement = document.querySelector('#paxInfoEmail');
		if (importantInfoEl && emailInputField && emailInputField?.value && leadPaxSection && window?.utag?.link) {
	...
	const optiInit = (importantInfoEl, emailInputField: HTMLInputElement) => {
		const leadPaxSection: HTMLElement = document.querySelector('[aria-label="ADULT 1"]');
		...
		const confirmEmailEl: HTMLElement = document.querySelector('.opti_email_entered');
		...
		const waitForEl = setInterval(() => {
			const passengerLink: HTMLElement = document.querySelector('#opti_email_confirm_container a');
			if (passengerLink) {
